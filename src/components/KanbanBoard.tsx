...
import { OppgaveStatus as OppgaveStatusEnum, oppgaveStatusTilTekst } from "@/utils/status-mapper"
...
interface KanbanBoardProps {
  ...
  onSaveOppgave: (oppgave: Oppgave) => void;
}

const columns: OppgaveStatus[] = [OppgaveStatus.IKKE_STARTET, OppgaveStatus.I_GANG, OppgaveStatus.FULLFORT]

export const KanbanBoard: FC<KanbanBoardProps> = ({ prosjekt, currentUser, onEditOppgave, onSaveOppgave }) => {
  const [oppgaver, setOppgaver] = useState<Oppgave[]>(prosjekt.oppgaver || [])
  const [oppgaveTilRedigering, setOppgaveTilRedigering] = useState<Oppgave | null>(null)
  const [isEditModalOpen, setIsEditModalOpen] = useState<boolean>(false)

  const [oppgaveTelling, setOppgaveTelling] = useState({
    total: oppgaver.length,
    ikkeStartet: oppgaver.filter(o => o.status === OppgaveStatus.IKKE_STARTET).length,
    iGang: oppgaver.filter(o => o.status === OppgaveStatus.I_GANG).length,
    fullfort: oppgaver.filter(o => o.status === OppgaveStatus.FULLFORT).length
  });
        ...
        setOppgaver((prevOppgaver) =>
          prevOppgaver.map((oppgave) =>
            oppgave.id === oppgaveId
              ? {
                  ...oppgave,
                  filer: oppgave.filer?.filter((fil) => fil.id !== filId),
                }
              : oppgave
          )
        );
          ...
          <Droppable key={status} droppableId={status}>
            {(provided) => (
              <Card className="w-full md:w-1/3">
                <CardHeader>
                  <CardTitle>{oppgaveStatusTilTekst[status as OppgaveStatus]}</CardTitle>
                </CardHeader>
                <CardContent 
